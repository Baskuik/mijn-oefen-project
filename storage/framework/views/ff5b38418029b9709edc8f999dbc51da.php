...
<body class="bg-slate-100 dark:bg-slate-950 min-h-screen transition-colors duration-300">
  ...
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 -mt-14 pb-16">
    ...
    <?php else: ?>
      <div class="space-y-4">
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $orders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
          <?php
          ...
          <div x-data="{ open: <?php echo e($loop->first ? 'true' : 'false'); ?> }"
            ...
            <button @click="open = !open" class="w-full text-left flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-5 hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors focus:outline-none">
              <div class="flex items-center gap-5">
                ...
                <div>
                  ...
                  <p class="font-bold text-slate-900 dark:text-white text-base">#<?php echo e($order->id); ?></p>
                </div>
                ...
                <div class="hidden sm:block">
                  ...
                  <p class="font-semibold text-slate-700 dark:text-slate-300 text-sm"><?php echo e($order->created_at->format('d M Y')); ?></p>
                </div>
                <div class="hidden sm:block w-px h-8 bg-slate-200 dark:bg-slate-600"></div>
                <div class="hidden sm:block">
                  <p class="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold">Artikelen</p>
                  <p class="font-semibold text-slate-700 dark:text-slate-300 text-sm"><?php echo e($order->items->count()); ?> <?php echo e($order->items->count() === 1 ? 'artikel' : 'artikelen'); ?></p>
                </div>
              </div>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="text-right">
                  ...
                  <p class="font-extrabold text-slate-900 dark:text-white text-lg">€<?php echo e(number_format($order->total_price, 2, ',', '.')); ?></p>
                </div>
            ...
            <div x-show="open"
                  ...
                  <div class="flex items-center justify-between px-6 py-4 gap-4 hover:bg-slate-50/60 dark:hover:bg-slate-700/30 transition-colors">
                    <div class="flex items-center gap-4 min-w-0">
                      <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($item->product?->image): ?>
                        <img src="<?php echo e(asset('storage/' . $item->product->image)); ?>" alt="<?php echo e($item->product->name); ?>" class="w-14 h-14 rounded-xl object-cover flex-shrink-0 border border-slate-100 dark:border-slate-600 shadow-sm">
                      <?php else: ?>
                      ...
                      <div class="min-w-0">
                        ...
                        <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5">
                          Aantal: <span class="font-semibold text-slate-600 dark:text-slate-300"><?php echo e($item->quantity); ?></span>
                          · €<?php echo e(number_format($item->price, 2, ',', '.')); ?> per stuk
                        </p>
                      </div>
                    </div>
                    <div class="text-right flex-shrink-0">
                      <p class="text-base font-bold text-slate-900 dark:text-white">€<?php echo e(number_format($item->price * $item->quantity, 2, ',', '.')); ?></p>
                    </div>
                  </div>
              ...
              <div class="flex items-center justify-between px-6 py-4 bg-slate-50 dark:bg-slate-700/40 border-t border-slate-100 dark:border-slate-700">
                <p class="text-xs text-slate-400 dark:text-slate-500">Besteld op <?php echo e($order->created_at->format('d F Y \o\m H:i')); ?></p>
                <div class="flex items-center gap-2">
                  <span class="text-sm text-slate-500 dark:text-slate-400 font-medium">Totaal:</span>
                  <span class="text-base font-extrabold text-slate-900 dark:text-white">€<?php echo e(number_format($order->total_price, 2, ',', '.')); ?></span>
                </div>
              </div>
            </div>
