<div data-pages='@json($this->previewPages)' x-data='{
        page: "",
        device: "desktop",
        allowClicks: false,
        scale: 1,
        ...
        init() {
            // Veilige initialisatie: parse uit data-attribute
    ...
    }' x-init="init()" class="block">
	<!-- Sticky header in Filament-stijl -->
	<div
		class="sticky top-0 z-20 border-b border-gray-200 dark:border-white/10 bg-white dark:bg-gray-900 shadow-sm fi-header">
		<div class="flex items-center justify-between gap-3 px-4 py-3">
			...
			<div class="flex items-center gap-2">
				...
				<x-filament::button color="gray" tag="a" target="_blank" icon="heroicon-o-arrow-top-right-on-square"
					x-bind:href="pages[page] || '#'">
					Openen
				</x-filament::button>

				<x-filament::button type="submit" form="edit-website-form" color="primary" icon="heroicon-o-check"
	...
	<div class="grid grid-cols-12 gap-6 px-4 py-4">
		...
		<div class="col-span-12 xl:col-span-7">
			<x-filament::section>
				<!-- Toolbar: Filament inputs & icon-buttons -->
				<div class="fi-section-header flex flex-wrap items-center gap-3">
					<div class="min-w-[14rem]">
						<x-filament::input.wrapper for="page-select" inline>
							...
							<x-filament::input.select id="page-select" x-model="$wire.page" wire:model.live="page"
								class="w-full">
					...
					<div class="ml-auto flex items-center gap-1">
						<!-- Let op: x-bind:class (geen :class) om Blade/PHP-evaluatie te vermijden -->
						<x-filament::icon-button icon="heroicon-o-computer-desktop" label="Desktop" size="sm"
							color="gray" x-on:click="device = 'desktop'"
							x-bind:class="device === 'desktop' ? 'text-primary-600 bg-primary-50 ring-1 ring-primary-600/20 dark:bg-primary-500/10' : ''" />
						<x-filament::icon-button icon="heroicon-o-device-tablet" label="Tablet" size="sm" color="gray"
						...
						<x-filament::icon-button icon="heroicon-o-device-phone-mobile" label="Mobiel" size="sm"
							color="gray" x-on:click="device = 'mobile'"
							x-bind:class="device === 'mobile' ? 'text-primary-600 bg-primary-50 ring-1 ring-primary-600/20 dark:bg-primary-500/10' : ''" />

						<x-filament::icon-button icon="heroicon-o-arrow-path" label="Vernieuwen" size="sm" color="gray"
							x-on:click="reloadPreview()" />
					</div>
				</div>

				<!-- Browser chrome + stage -->
				<div class="mt-3 overflow-hidden rounded-xl border border-gray-200 dark:border-white/10">
					<div
						class="flex items-center gap-3 border-b border-gray-200 bg-gray-50 px-3 py-2 dark:border-white/10 dark:bg-white/5">
						<!-- traffic lights -->
						...
						<div class="flex-1 truncate text-[11px] text-gray-600 dark:text-gray-400"
					...
					<div id="preview-stage" class="bg-gray-100 p-4 dark:bg-gray-900">
						<div x-bind:style="'height: ' + (iframeHeight * scale) + 'px;'">
							<iframe id="site-preview-iframe" x-bind:src="pages[page] || ''"
								...
								title="Webshop live preview"></iframe>
						</div>
					</div>

					<!-- Statusbalk -->
					<div
						class="flex items-center justify-between border-t border-gray-200 bg-gray-50 px-3 py-1.5 text-[11px] text-gray-600 dark:border-white/10 dark:bg-white/5 dark:text-gray-400">
						...
						<label class="inline-flex cursor-pointer items-center gap-1.5">
							<input type="checkbox" x-model="allowClicks"
								class="h-3.5 w-3.5 rounded border-gray-300 text-primary-600 focus:ring-0 dark:border-white/10">
