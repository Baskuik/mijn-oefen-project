<div
    x-data="{
        ...
        init() {
            ...
            // Na opslaan: iframe herladen
            window.addEventListener('site-settings-saved', () => this.reloadPreview());

            // Paginalijst opnieuw opgebouwd in PHP → hier bijwerken
...
>
    ...
    <div class="sticky top-0 z-30 mb-5 flex items-center justify-between
                border-b border-gray-200/80 bg-white/90 px-1 py-3 backdrop-blur-md">
        ...
        <div class="flex items-center gap-2">
            <button
                ...
                class="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white
                       px-3 py-1.5 text-xs font-medium text-gray-600 shadow-sm hover:bg-gray-50"
                title="Paginalijst opnieuw scannen"
            >
                {{-- Arrow Path (refresh) --}}
                ...
                <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="flex items-center gap-3 border-t border-gray-100 bg-gray-50/60 px-4 py-3">
                        ...
                        <p class="text-[11px] text-gray-400 leading-tight">
                            Preview vernieuwt automatisch na het opslaan.
                        </p>
                    </div>
                </div>
            </form>
            ...
            <div class="xl:sticky xl:top-16 rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
                ...
                <div class="flex flex-wrap items-center gap-2 border-b border-gray-100 bg-gray-50 px-3 py-2">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select
                            x-model="page"
                            class="min-w-[160px] rounded-md border border-gray-300 bg-white
                                   px-2.5 py-1.5 text-[12px] shadow-sm
                    ...
                    <div class="ml-auto flex items-center gap-0.5 rounded-md bg-gray-200/70 p-0.5">
                        {{-- Desktop --}}
                        <button type="button" x-on:click="device = 'desktop'"
                                :class="device === 'desktop' ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'"
                                class="rounded p-1 transition" title="Desktop">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                <rect x="2" y="3" width="20" height="14" rx="2"/><path d="M8 21h8M12 17v4"/>
                            </svg>
                        </button>
                        {{-- Tablet --}}
                        <button type="button" x-on:click="device = 'tablet'"
                                :class="device === 'tablet' ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'"
                                class="rounded p-1 transition" title="Tablet">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                                <rect x="4" y="2" width="16" height="20" rx="2"/><circle cx="12" cy="18" r="1"/>
                            </svg>
                        </button>
                        {{-- Mobile --}}
                        <button type="button" x-on:click="device = 'mobile'"
                                :class="device === 'mobile' ? 'bg-white shadow text-indigo-600' : 'text-gray-500 hover:text-gray-700'"
                                class="rounded p-1 transition" title="Mobiel">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                ...
                {{-- Statusbalk --}}
                <div class="flex flex-wrap items-center justify-between gap-2 border-t border-gray-100
