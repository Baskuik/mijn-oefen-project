...
<head>
  ...
  <meta name="csrf-token" content="{{ csrf_token() }}"><!-- IMPORTANT for Livewire/axios -->
          ...
          <div class="mb-20" 
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              @foreach($category->products as $product)
                <div x-show="searchQuery === '' || '{{ strtolower($product->name) }} {{ strtolower($product->description ?? '') }}'.includes(searchQuery.toLowerCase())"
                     x-transition
