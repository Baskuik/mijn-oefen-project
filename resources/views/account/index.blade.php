...
<body class="bg-slate-100 dark:bg-slate-950 min-h-screen transition-colors duration-300">
  ...
  <div
    x-data="{ showConfirm:false, showEmailForm:false, newEmail:'', isVerified: {{ $user->email_verified_at ? 'true':'false' }} }"
  >
    <!-- Hero -->
    <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-900 pt-12 pb-24">
      <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <div class="mt-4 flex items-center justify-center gap-4 text-sm">
          <a href="{{ route('orders.index') }}" class="inline-flex items-center gap-1.5 text-indigo-300 hover:text-white transition-colors">Mijn Bestellingen</a>
          <span class="text-white/20">|</span>
          <a href="{{ route('home') }}" class="inline-flex items-center gap-1.5 text-slate-300 hover:text-white transition-colors">Terug naar shop</a>
        </div>
      </div>
    </div>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 -mt-16 pb-16">
      ...
      @if(session('status') === 'email-updated')
        <div class="mb-4 flex items-center gap-3 bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400 px-5 py-4 rounded-xl shadow-sm">
      ...
      <!-- Email card -->
      <div class="card-animate bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 mb-5 overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-indigo-50 to-slate-50 dark:from-indigo-900/30 dark:to-slate-900/30 border-b border-slate-100 dark:border-slate-700">
          ...
          <div>
            <h2 class="text-base font-bold text-slate-900 dark:text-white">E-mailadres</h2>
            <p class="text-xs text-slate-400 dark:text-slate-500">Beheer je inlogadres</p>
          </div>
        </div>

        <div class="px-6 py-6">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-600">
            <div>
              <p class="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mb-0.5">Huidig e-mailadres</p>
              <p class="text-slate-800 dark:text-white font-semibold">{{ $user->email }}</p>
            </div>
            <div class="flex-shrink-0">
              @if($user->email_verified_at)
                <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100 dark:bg-emerald-900/40 border border-emerald-200 dark:border-emerald-700 text-emerald-700 dark:text-emerald-400 text-xs font-bold rounded-full">
                  Geverifieerd
                </span>
              @else
                <div class="flex flex-col gap-2 items-start sm:items-end">
                  <span class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-amber-100 dark:bg-amber-900/40 border border-amber-200 dark:border-amber-700 text-amber-700 dark:text-amber-400 text-xs font-bold rounded-full">
                    Niet geverifieerd
                  </span>
                  <form method="POST" action="{{ route('verification.send') }}">
                    @csrf
                    <button type="submit" class="text-xs text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 font-medium underline underline-offset-2 transition-colors">
                      Verificatie opnieuw sturen →
                    </button>
                  </form>
                  @if(session('status') === 'verification-link-sent')
                    <p class="text-xs text-emerald-600 dark:text-emerald-400 font-medium">Verstuurd!</p>
                  @endif
                </div>
              @endif
            </div>
          ...
          <div x-show="showEmailForm"
            ...
            <form method="POST" action="{{ route('account.email.update') }}" class="mt-4 p-4 bg-indigo-50 dark:bg-slate-700 rounded-xl border border-indigo-100 dark:border-slate-600 space-y-4">
              @csrf
              @method('PATCH')
              <div>
                <label for="email" class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Nieuw e-mailadres</label>
                <input id="email" name="email" type="email" required autofocus autocomplete="email"
                       x-model="newEmail" placeholder="{{ $user->email }}"
                       class="w-full px-4 py-2.5 bg-white dark:bg-slate-800 border border-indigo-200 dark:border-indigo-700 rounded-lg text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm shadow-sm transition-all">
                @error('email')
              ...
              <div class="flex items-center gap-3">
                ...
                <button type="button" @click="showEmailForm = false; newEmail = ''" class="px-4 py-2.5 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors">
                  Annuleren
                </button>
              </div>
            </form>
      ...
      <div x-data="{ showPasswordForm:false, showCurrent:false, showNew:false, showConfirmPw:false }"
           class="card-animate bg-white dark:bg-slate-800 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 bg-gradient-to-r from-slate-100 to-slate-50 dark:from-slate-700/50 dark:to-slate-800 border-b border-slate-100 dark:border-slate-700">
          <div class="w-10 h-10 bg-slate-700 dark:bg-slate-600 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm">
          ...
          <div>
            <h2 class="text-base font-bold text-slate-900 dark:text-white">Wachtwoord</h2>
            <p class="text-xs text-slate-400 dark:text-slate-500">Bescherm je account</p>
          </div>
        </div>

        <div class="px-6 py-6">
          <div class="mb-6" x-show="!showPasswordForm">
            <div class="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-600">
              ...
              <div>
                <p class="text-xs text-slate-400 dark:text-slate-500 uppercase tracking-wider font-semibold mb-1">Wachtwoord</p>
                <span class="password-dots">● ● ● ● ● ● ● ●</span>
              </div>
              <span class="ml-auto text-xs text-slate-400 dark:text-slate-300 bg-slate-200 dark:bg-slate-600 px-2 py-1 rounded-md font-medium">Verborgen</span>
            </div>
          </div>

          <button x-show="!showPasswordForm" @click="showPasswordForm = true"
                  class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 dark:bg-slate-600 text-white text-sm font-semibold rounded-xl hover:bg-slate-700 dark:hover:bg-slate-500 active:scale-95 transition-all shadow-sm">
          ...
          <div x-show="showPasswordForm"
               ...
               x-transition:enter-start="opacity-0 -translate-y-2"
               x-transition:enter-end="opacity-100 translate-y-0">
            <form method="POST" action="{{ route('password.update') }}" class="mt-4 p-4 bg-slate-50 dark:bg-slate-700 rounded-xl border border-slate-100 dark:border-slate-600 space-y-4">
              @csrf
              @method('PUT')

              <div>
                <label for="current_password" class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Huidig wachtwoord</label>
                <div class="relative">
                  <input id="current_password" name="current_password" :type="showCurrent ? 'text' : 'password'" required autocomplete="current-password"
                         class="w-full px-4 py-2.5 pr-11 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-slate-800 dark:focus:ring-indigo-500 focus:border-transparent text-sm shadow-sm transition-all">
                  <button type="button" @click="showCurrent = !showCurrent" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                ...
                @error('current_password', 'updatePassword') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p> @enderror
              </div>

              <div>
                <label for="password" class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Nieuw wachtwoord</label>
                <div class="relative">
                  <input id="password" name="password" :type="showNew ? 'text' : 'password'" required autocomplete="new-password"
                         class="w-full px-4 py-2.5 pr-11 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-slate-800 dark:focus:ring-indigo-500 focus:border-transparent text-sm shadow-sm transition-all">
                  <button type="button" @click="showNew = !showNew" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
                    <svg x-show="!showNew" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                    <svg x-show="showNew" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268-2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21"/></svg>
                  </button>
                </div>
                @error('password', 'updatePassword') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p> @enderror
              </div>

              <div>
                <label for="password_confirmation" class="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Bevestig nieuw wachtwoord</label>
                <div class="relative">
                  <input id="password_confirmation" name="password_confirmation" :type="showConfirmPw ? 'text' : 'password'" required autocomplete="new-password"
                         class="w-full px-4 py-2.5 pr-11 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-slate-800 dark:focus:ring-indigo-500 focus:border-transparent text-sm shadow-sm transition-all">
                  <button type="button" @click="showConfirmPw = !showConfirmPw" class="absolute inset-y-0 right-0 pr-3 flex items-center text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition-colors">
              ...
              <div class="flex items-center gap-3 pt-1">
                <button type="submit" class="inline-flex items-center gap-2 px-5 py-2.5 bg-slate-800 dark:bg-slate-600 text-white text-sm font-semibold rounded-xl hover:bg-slate-700 dark:hover:bg-slate-500 active:scale-95 transition-all shadow-sm">
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                  Opslaan
                </button>
                <button type="button" @click="showPasswordForm = false" class="px-4 py-2.5 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white transition-colors">
                  Annuleren
                </button>
              </div>
            </form>
        ...
        <div class="px-6 py-5 space-y-4">
          ...
          <div class="flex items-center gap-3 px-4 py-3 bg-slate-50 dark:bg-slate-700/60 rounded-xl border border-slate-200 dark:border-slate-600">
            ...
            <div class="min-w-0">
              ...
              <p class="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{{ $user->email }}</p>
            </div>
